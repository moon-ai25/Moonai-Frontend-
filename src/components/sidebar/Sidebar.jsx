import React, { useEffect, useRef, useState } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { Plus, LogOut, Settings, Search, PanelLeft, SquarePen, MessageCircle } from 'lucide-react'
import useChatStore from '../../store/chatStore'
import useAuth from '../../hooks/useAuth'
import useChat from '../../hooks/useChat'
import useSearch from '../../hooks/useSearch'
import { getChatsByUsername, getChatById } from '../../api/moonai'
import ChatHistoryItem from './ChatHistoryItem'
import { groupChatsByDate } from '../../utils/dateHelpers'
import toast from 'react-hot-toast'
import NewChatIcon from '../icons/NewChatIcon'

const MoonLogo = ({ size = 28 }) => (
  <svg width={size} height={size} viewBox="0 0 1024 1024">
    <path fill="var(--primary-color)" d="M589.090759,165.440460 C589.857056,171.388962 586.072571,174.014038 582.015320,176.120468 C572.702698,180.955185 563.217468,185.456161 553.877258,190.239044 C513.945312,210.687241 480.423676,238.327866 456.827301,277.044739 C441.094421,302.859222 431.573486,330.882355 427.328003,360.737396 C425.313385,374.904419 424.037048,389.129852 424.262177,403.476593 C424.982880,449.402313 436.764282,492.251831 460.279694,531.708496 C485.251526,573.608887 519.317810,605.776794 563.861755,626.242981 C583.467590,635.251038 604.016785,641.099243 625.513977,643.924255 C638.970093,645.692688 652.421326,646.247742 665.866821,645.619690 C719.931458,643.094177 768.919739,626.480347 811.317383,592.095093 C815.456848,588.737915 819.583069,585.352112 823.884216,582.210754 C826.775940,580.098755 829.996277,578.503723 833.506409,581.155640 C836.995239,583.791504 837.761414,587.412231 836.767151,591.412537 C834.668335,599.857178 831.231384,607.831970 827.434387,615.606445 C801.834106,668.024353 764.486755,710.098328 715.861877,742.293518 C680.074402,765.988770 640.977539,781.346680 598.856262,788.995605 C571.383911,793.984436 543.641357,795.139648 515.850342,792.371399 C438.538605,784.670105 371.032837,754.356384 313.882843,702.032227 C264.882568,657.169617 232.851212,601.893860 218.121033,536.906677 C212.734177,513.140808 210.088257,489.012421 210.831177,464.650818 C212.766388,401.191437 231.023453,343.251160 269.749847,292.297943 C299.164825,253.595993 334.537079,221.611725 376.876953,197.662476 C411.369232,178.152130 448.218964,164.912933 487.465698,158.837402 C516.549866,154.335037 545.752197,153.735703 574.940002,157.809387 C580.239563,158.549057 586.007690,159.364197 589.090759,165.440460 M503.487366,759.822205 C510.056763,760.780090 516.599304,762.254028 523.200684,762.597839 C542.141357,763.584412 561.069946,763.408447 579.916504,760.698547 C611.605530,756.142273 641.936768,747.179016 670.858765,733.394836 C710.994568,714.266174 745.199707,687.358337 774.850830,654.449524 C778.302490,650.618652 781.622986,646.645508 784.714111,642.520935 C787.749512,638.470703 791.535828,634.892578 793.610107,629.567688 C791.328064,630.977539 789.427856,632.022949 787.662781,633.261047 C751.420288,658.682373 710.496826,670.286072 666.920105,673.093933 C640.383179,674.803772 613.900635,672.424072 588.448242,664.441467 C553.370789,653.440186 518.513672,641.698425 483.786346,629.628845 C445.637268,616.369934 410.325623,597.619629 377.637024,573.732910 C367.962006,566.663025 360.814270,559.272827 360.145355,546.714233 C359.841827,541.015747 358.179688,535.389526 358.939056,528.316650 C377.762909,544.532654 396.485443,559.337280 417.385010,571.160828 C438.283661,582.983887 459.809052,593.349121 483.610657,602.106262 C438.008453,561.702759 413.102264,511.301544 402.144379,453.994415 C391.107330,396.273346 399.310791,340.721710 427.195709,288.373993 C425.144135,289.806549 423.565094,291.296844 422.305054,293.020355 C411.075592,308.379913 400.968903,324.459656 392.581757,341.536194 C363.999756,399.730164 350.837433,461.116974 354.136719,526.037598 C355.280670,548.546692 358.327362,570.730652 363.488770,592.569519 C373.906281,636.647705 391.038971,677.587158 420.092072,713.097412 C441.705811,739.514771 468.647766,755.647766 503.487366,759.822205 M299.188477,361.290619 C297.731323,375.731232 298.025482,390.142151 299.201355,404.609467 C300.969055,426.357819 305.999176,447.253052 314.544434,467.277771 C318.173248,475.781372 322.467682,483.992462 328.246155,492.436218 C335.293243,349.754608 409.357483,250.718948 526.911621,179.338074 C515.080383,178.836960 503.505402,181.491196 491.973663,183.756409 C437.869354,194.384247 389.029175,216.444031 346.888641,252.336731 C332.461548,264.624847 318.679993,277.311768 312.668793,296.680206 C306.185547,317.569763 300.458710,338.444183 299.188477,361.290619 M241.415588,486.045166 C242.412964,493.947601 243.170578,501.889008 244.447983,509.745941 C251.033264,550.249756 265.667786,587.691406 288.205597,621.936829 C307.910278,651.877380 332.932281,676.885864 361.383545,698.507874 C368.861328,704.190735 376.428833,709.845581 385.711395,714.430176 C385.158447,713.021362 385.040985,712.388000 384.703308,711.910339 C374.949280,698.111511 367.385529,683.136597 360.585449,667.712036 C343.184235,628.241028 332.485443,587.116455 329.679199,544.015198 C329.430939,540.202454 328.083008,537.127563 325.565033,534.159790 C294.346771,497.364410 276.117401,454.999054 272.943024,406.765167 C270.962311,376.668304 273.656677,346.899445 284.600922,318.357666 C285.196411,316.804657 286.228821,315.135742 285.008392,313.021057 C281.782501,315.554321 280.591705,319.128998 278.723816,322.185608 C257.572479,356.797607 247.289093,394.957703 242.342880,434.791534 C240.267441,451.505829 240.867813,468.323456 241.415588,486.045166 z" />
  </svg>
)

const SidebarSkeleton = () => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '4px 8px' }}>
    {[100, 85, 90, 75, 95, 80].map((w, i) => (
      <div
        key={i}
        className="skeleton-sidebar-item"
        style={{ width: `${w}%`, height: 36, borderRadius: 8 }}
      />
    ))}
  </div>
)

function NavItem({ icon, label, onClick }) {
  return (
    <button
      onClick={onClick}
      style={{
        display: 'flex',
        alignItems: 'center',
        gap: 12,
        width: '100%',
        padding: '9px 12px',
        background: 'transparent',
        border: 'none',
        borderRadius: 8,
        cursor: 'pointer',
        color: 'var(--text-primary)',
        fontSize: 14,
        fontFamily: 'var(--font-body)',
        fontWeight: 400,
        textAlign: 'left',
        transition: 'background 0.15s',
        margin: '1px 6px',
        boxSizing: 'border-box',
        width: 'calc(100% - 12px)',
      }}
      onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-hover)'}
      onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
    >
      <span style={{ color: 'var(--text-primary)', display: 'flex', alignItems: 'center', flexShrink: 0 }}>{icon}</span>
      {label}
    </button>
  )
}

export default function Sidebar({ isOpen }) {
  const { user } = useChatStore()
  const { chatList, setChatList, isLoadingChats, setLoadingChats, currentTitle, newChat: resetChat, setSettingsOpen, toggleSidebar } = useChatStore()
  const { loadChat, startNewChat } = useChat()
  const { openSearch } = useSearch()
  const { logout } = useAuth()
  const loadedRef = useRef(false)
  const activeChatReqRef = useRef(null)

  const [userMenuOpen, setUserMenuOpen] = useState(false)
  const [showRecentFloating, setShowRecentFloating] = useState(false)
  const [isSearching, setIsSearching] = useState(false)
  const [localSearchQuery, setLocalSearchQuery] = useState('')
  const userMenuRef = useRef(null)

  useEffect(() => {
    if (!userMenuOpen) return
    const handler = (e) => {
      if (userMenuRef.current && !userMenuRef.current.contains(e.target)) {
        setUserMenuOpen(false)
      }
    }
    document.addEventListener('mousedown', handler)
    return () => document.removeEventListener('mousedown', handler)
  }, [userMenuOpen])

  useEffect(() => {
    if (!user?.username || loadedRef.current) return
    loadedRef.current = true
    setLoadingChats(true)
    getChatsByUsername(user.username)
      .then((data) => setChatList(data.chats || data || []))
      .catch(() => { })
      .finally(() => setLoadingChats(false))
  }, [user, setChatList, setLoadingChats])

  const handleChatClick = async (chat) => {
    try {
      const targetId = chat._id || chat.chatId
      activeChatReqRef.current = targetId

      // Immediate optimistic update to clear current chat & show new title
      loadChat({ _id: targetId, title: chat.title, messages: [] })
      setShowRecentFloating(false)

      const full = await getChatById(user.username, targetId)

      // Only load messages if this is still the active clicked chat
      if (activeChatReqRef.current === targetId) {
        if (full) {
          loadChat(full)
        } else {
          toast.error('Chat data could not be loaded')
        }
      }
    } catch {
      if (activeChatReqRef.current === (chat._id || chat.chatId)) {
        toast.error('Failed to load chat')
      }
    }
  }

  const grouped = groupChatsByDate(chatList)
  const displayedChats = localSearchQuery.trim()
    ? chatList.filter(c => c.title.toLowerCase().includes(localSearchQuery.toLowerCase()))
    : chatList
  const hasChats = chatList.length > 0

  // ── Collapsed Sidebar ───────────────────────────────────────────────────────
  if (!isOpen) {
    return (
      <div style={{
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
        height: '100%',
        width: 60,
        background: 'var(--bg-primary)',
        borderRight: '1px solid var(--border-subtle)',
        padding: '16px 0',
        gap: 6,
        position: 'relative',
      }}>
        <IconBtn title="Expand sidebar" onClick={toggleSidebar}><MoonLogo size={36} /></IconBtn>
        <div style={{ height: 8 }} />
        <IconBtn title="New chat" onClick={startNewChat}><NewChatIcon size={20} /></IconBtn>
        <IconBtn title="Search chats" onClick={openSearch}><Search size={20} strokeWidth={1.8} /></IconBtn>

        {/* Recents popup button */}
        <div style={{ position: 'relative' }}>
          <IconBtn title="Recent chats" onClick={() => setShowRecentFloating(!showRecentFloating)}>
            <MessageCircle size={20} strokeWidth={1.8} color={showRecentFloating ? 'var(--text-primary)' : 'currentColor'} />
          </IconBtn>

          {/* Floating Recent Container */}
          <AnimatePresence>
            {showRecentFloating && (
              <>
                <div
                  style={{ position: 'fixed', inset: 0, zIndex: 9998 }}
                  onClick={() => setShowRecentFloating(false)}
                />
                <motion.div
                  initial={{ opacity: 0, x: -10, scale: 0.95 }}
                  animate={{ opacity: 1, x: 0, scale: 1 }}
                  exit={{ opacity: 0, x: -10, scale: 0.95 }}
                  style={{
                    position: 'absolute',
                    left: 56,
                    top: 0,
                    width: 280,
                    maxHeight: '70vh',
                    background: 'var(--bg-elevated)',
                    border: '1px solid var(--border-medium)',
                    borderRadius: 'var(--radius-lg)',
                    boxShadow: 'var(--shadow-card)',
                    zIndex: 9999,
                    display: 'flex',
                    flexDirection: 'column',
                    overflow: 'hidden',
                  }}
                >
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-subtle)', fontWeight: 600, fontSize: 13, color: 'var(--text-primary)' }}>
                    Recent Conversations
                  </div>
                  <div style={{ overflow: 'auto', padding: '8px' }}>
                    {isLoadingChats ? (
                      <SidebarSkeleton />
                    ) : hasChats ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {chatList.slice(0, 10).map((chat) => (
                          <div
                            key={chat._id || chat.id}
                            onClick={() => handleChatClick(chat)}
                            style={{
                              padding: '10px 12px',
                              borderRadius: 'var(--radius-sm)',
                              cursor: 'pointer',
                              fontSize: 13,
                              color: chat.title === currentTitle ? 'var(--text-primary)' : 'var(--text-secondary)',
                              background: chat.title === currentTitle ? 'var(--bg-hover)' : 'transparent',
                              transition: 'all 0.2s',
                              overflow: 'hidden',
                              textOverflow: 'ellipsis',
                              whiteSpace: 'nowrap',
                            }}
                            onMouseEnter={(e) => { if (chat.title !== currentTitle) e.currentTarget.style.background = 'rgba(255,255,255,0.03)' }}
                            onMouseLeave={(e) => { if (chat.title !== currentTitle) e.currentTarget.style.background = 'transparent' }}
                          >
                            {chat.title}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 12 }}>
                        No recent chats
                      </div>
                    )}
                  </div>
                </motion.div>
              </>
            )}
          </AnimatePresence>
        </div>

        <div style={{ flex: 1 }} />
        {user && (
          <button
            onClick={() => setSettingsOpen(true)}
            title={user.name || user.username}
            style={{
              width: 34, height: 34, borderRadius: '50%',
              background: 'var(--primary-color)',
              border: '2px solid var(--border-medium)',
              cursor: 'pointer', padding: 0, overflow: 'hidden',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 13, fontWeight: 700,
              color: 'var(--bg-primary)',
              fontFamily: 'var(--font-display)',
              flexShrink: 0,
            }}
          >
            {user.profile ? (
              <img
                src={user.profile}
                alt={user.name || user.username}
                style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '50%' }}
                onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex' }}
              />
            ) : null}
            <span style={{ display: user.profile ? 'none' : 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: '100%' }}>
              {(user.name || user.username)?.[0]?.toUpperCase() || '?'}
            </span>
          </button>
        )}
      </div>
    )
  }

  // ── Full sidebar ────────────────────────────────────────────────────────────
  return (
    <div
      className="sidebar-animate"
      style={{
        display: 'flex',
        flexDirection: 'column',
        height: '100%',
        width: 260,
        background: 'var(--bg-primary)',
        borderRight: '1px solid var(--border-subtle)',
        overflow: 'hidden',
      }}
    >
      {/* ── Top bar: logo ── */}
      <div style={{
        display: 'flex',
        alignItems: 'center',
        padding: '16px 16px 12px',
        flexShrink: 0,
      }}>
        <div style={{ padding: '0 4px', display: 'flex', alignItems: 'center' }}>
          <MoonLogo size={46} />
        </div>
      </div>

      {/* ── Action Buttons (Search & New Chat) ── */}
      {isSearching ? (
        <div style={{ padding: '12px 16px', display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
          <div style={{ flex: 1, background: 'var(--bg-elevated)', borderRadius: 22, display: 'flex', alignItems: 'center', padding: '0 12px', border: '1px solid var(--border-subtle)', height: 44 }}>
            <Search size={18} color="var(--text-tertiary)" />
            <input
              autoFocus
              value={localSearchQuery}
              onChange={e => setLocalSearchQuery(e.target.value)}
              placeholder="Search..."
              style={{ background: 'transparent', border: 'none', color: 'var(--text-primary)', padding: '12px 8px', width: '100%', outline: 'none', fontSize: 14 }}
            />
          </div>
          <button
            onClick={() => { setIsSearching(false); setLocalSearchQuery(''); }}
            style={{ background: 'transparent', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', padding: 8 }}
          >
            <X size={20} />
          </button>
        </div>
      ) : (
        <div style={{ padding: '12px 16px', display: 'flex', gap: 12, alignItems: 'center', flexShrink: 0 }}>
          <button
            onClick={() => setIsSearching(true)}
            style={{
              width: 44,
              height: 44,
              borderRadius: '50%',
              background: 'var(--bg-elevated)',
              border: '1px solid var(--border-subtle)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              color: 'var(--text-primary)',
              cursor: 'pointer',
              flexShrink: 0,
              transition: 'background 0.2s',
            }}
            onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-hover)'}
            onMouseLeave={(e) => e.currentTarget.style.background = 'var(--bg-elevated)'}
          >
            <Search size={20} strokeWidth={1.8} />
          </button>

          <button
            onClick={startNewChat}
            style={{
              flex: 1,
              height: 44,
              borderRadius: 22,
              background: 'var(--text-primary)',
              color: 'var(--bg-primary)',
              border: 'none',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              gap: 8,
              fontSize: 15,
              fontWeight: 600,
              cursor: 'pointer',
              transition: 'opacity 0.2s',
            }}
            onMouseEnter={(e) => e.currentTarget.style.opacity = 0.9}
            onMouseLeave={(e) => e.currentTarget.style.opacity = 1}
          >
            <NewChatIcon size={18} color="var(--bg-primary)" />
            New chat
          </button>
        </div>
      )}

      {/* ── Chat list ── */}
      <div style={{
        flex: 1,
        overflowY: 'auto',
        overflowX: 'hidden',
        padding: '8px 0 80px', // Extra bottom padding for floating profile overlay
        WebkitMaskImage: 'linear-gradient(to bottom, transparent, black 16px, black calc(100% - 16px), transparent)',
        maskImage: 'linear-gradient(to bottom, transparent, black 16px, black calc(100% - 16px), transparent)',
      }}>
        <div style={{ padding: '12px 16px 8px', fontSize: 13, fontWeight: 700, color: 'var(--text-tertiary)', fontFamily: 'var(--font-body)' }}>
          Recent Chats
        </div>

        {isLoadingChats ? (
          <SidebarSkeleton />
        ) : hasChats ? (
          /* Flat list — no folder groups */
          displayedChats.map((chat) => (
            <ChatHistoryItem
              key={chat._id || chat.title}
              chat={chat}
              isActive={chat.title === currentTitle}
              onClick={handleChatClick}
              username={user?.username}
            />
          ))
        ) : (
          <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 13 }}>
            {localSearchQuery ? 'No matching chats' : 'No conversations yet'}
          </div>
        )}
      </div>

      {/* ── Footer: user ── */}
      <div style={{
        padding: '10px 12px 14px',
        flexShrink: 0,
        position: 'absolute',
        bottom: 0,
        left: 0,
        width: '100%',
        maxWidth: '100%',
        zIndex: 10,
      }}>
        {user && (
          <>
            <div
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 12,
                padding: '6px 14px 6px 6px',
                borderRadius: 24,
                background: 'var(--bg-elevated)',
                border: '1px solid var(--border-subtle)',
                cursor: 'pointer',
                transition: 'background 0.15s',
                width: 'fit-content',
              }}
              onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-hover)'}
              onMouseLeave={(e) => e.currentTarget.style.background = 'var(--bg-elevated)'}
              onClick={() => setUserMenuOpen((v) => !v)}
            >
              {/* Avatar — real Google photo or initial fallback */}
              <div style={{
                width: 34, height: 34,
                borderRadius: '50%',
                background: 'var(--primary-color)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontWeight: 700, fontSize: 13,
                color: 'var(--bg-primary)',
                flexShrink: 0,
                fontFamily: 'var(--font-display)',
                overflow: 'hidden',
                border: '2px solid var(--border-subtle)',
              }}>
                {user.profile ? (
                  <img
                    src={user.profile}
                    alt={user.name || user.username}
                    style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                    onError={(e) => { e.target.style.display = 'none' }}
                  />
                ) : (
                  (user.name || user.username)?.[0]?.toUpperCase() || '?'
                )}
              </div>

              {/* Username only */}
              <div style={{ overflow: 'hidden' }}>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {user.name || user.username}
                </div>
              </div>
            </div>

            {/* Dropdown — Settings + Logout only */}
            <AnimatePresence>
              {userMenuOpen && (
                <motion.div
                  ref={userMenuRef}
                  initial={{ opacity: 0, y: 6, scale: 0.97 }}
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: 6, scale: 0.97 }}
                  transition={{ duration: 0.13, ease: 'easeOut' }}
                  style={{
                    position: 'absolute',
                    bottom: '100%',
                    left: 12,
                    right: 12,
                    marginBottom: 6,
                    background: 'var(--bg-elevated)',
                    border: '1px solid var(--border-medium)',
                    borderRadius: 12,
                    boxShadow: '0 -4px 24px rgba(0,0,0,0.35)',
                    padding: '5px',
                    zIndex: 200,
                  }}
                >
                  <button
                    onClick={() => { setUserMenuOpen(false); setSettingsOpen(true); }}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 9,
                      width: '100%', padding: '9px 11px',
                      background: 'none', border: 'none', borderRadius: 8,
                      cursor: 'pointer', textAlign: 'left',
                      color: 'var(--text-primary)', fontSize: 13.5, fontFamily: 'var(--font-body)',
                      fontWeight: 500, transition: 'background 0.13s',
                    }}
                    onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-hover)'}
                    onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                  >
                    <Settings size={14} strokeWidth={1.9} style={{ color: 'var(--text-secondary)', flexShrink: 0 }} />
                    Settings
                  </button>

                  <div style={{ height: 1, background: 'var(--border-subtle)', margin: '3px 6px' }} />

                  <button
                    onClick={() => { setUserMenuOpen(false); logout(); }}
                    style={{
                      display: 'flex', alignItems: 'center', gap: 9,
                      width: '100%', padding: '9px 11px',
                      background: 'none', border: 'none', borderRadius: 8,
                      cursor: 'pointer', textAlign: 'left',
                      color: '#f87171', fontSize: 13.5, fontFamily: 'var(--font-body)',
                      fontWeight: 500, transition: 'background 0.13s',
                    }}
                    onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(239,68,68,0.08)'}
                    onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                  >
                    <LogOut size={14} strokeWidth={1.9} style={{ flexShrink: 0 }} />
                    Log out
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
          </>
        )}
      </div>
    </div>
  )
}

function IconBtn({ children, onClick, title }) {
  return (
    <button
      onClick={onClick}
      title={title}
      style={{
        background: 'none', border: 'none', cursor: 'pointer',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        padding: 8, borderRadius: 8,
        color: 'var(--text-primary)',
        transition: 'background 0.15s',
      }}
      onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-hover)'}
      onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
    >
      {children}
    </button>
  )
}

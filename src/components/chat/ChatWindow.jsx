...
import { welcomeLogoEntrance } from '../../animations/gsapPresets'
import { getGreeting } from '../../utils/dateHelpers'
import TempChatIcon from '../icons/TempChatIcon'
import AlienIcon from '../icons/AlienIcon'

const MoonWelcomeLogo = () => (
  <svg
    width="110"
    height="110"
    viewBox="0 0 1024 1024"
    style={{ filter: 'drop-shadow(0 0 28px var(--primary-transparent))' }}
  >
...
export default function ChatWindow({ onSendSuggestion, attachedFiles }) {
  const { messages, isGenerating, currentChatId, editAndResend, regenerateLastResponse } = useChat()
  const { user, isSearchOpen, isSidebarOpen, isTempChat } = useChatStore()
  const { isPinned, pin } = usePinned(currentChatId)
  ...
  // Handle scroll detection for 'New Message' button
  const handleScroll = useCallback(() => {
    const el = scrollRef.current
    if (!el) return

    // In normal column, at bottom means scrollTop + clientHeight approx scrollHeight
    const atBottom = el.scrollHeight - el.scrollTop - el.clientHeight < 50
    setIsAtBottom(atBottom)
    if (atBottom) setShowNewMsgBtn(false)
  }, [])

  // Auto-scroll nudge for streaming and new user messages
  useEffect(() => {
    if (!scrollRef.current) return;
    
    const lastMsg = messages[messages.length - 1];
    const isJustSentByUser = lastMsg && lastMsg.role === 'user';
    
    if (isJustSentByUser || (isAtBottom && isGenerating)) {
  ...
  // Robust scroll to last user input on chat load
  useEffect(() => {
    const isNewChat = currentChatId && lastScrolledChatIdRef.current !== currentChatId
    if (isNewChat && messages.length > 0) {
      ...
      if (userMsgs.length > 0) {
        ...
        // Multiple attempts to ensure we settle correctly after all components render
        performScroll()
        const t1 = setTimeout(performScroll, 100)
        const t2 = setTimeout(performScroll, 300)
        const t3 = setTimeout(performScroll, 800)
        const t4 = setTimeout(performScroll, 1500) // Final insurance for slow renders
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          display: 'flex',
          flexDirection: 'column', // STANDARD LAYOUT
        ...
        {isEmpty ? (
          <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            >
              ...
              >
                {isTempChat ? (
                  <div style={{ color: 'var(--text-primary)', display: 'flex', justifyContent: 'center' }}>
                    <TempChatIcon size={80} active={true} color="var(--text-primary)" />
                  </div>
                ) : (
                  <MoonWelcomeLogo />
                )}
              ...
              <motion.h2
                ...
                style={{
                  fontFamily: isTempChat ? "'Pixelify Sans', var(--font-display)" : "'Pixelify Sans', var(--font-display)",
                  fontSize: 32,
                  fontWeight: 700,
                  letterSpacing: isTempChat ? '0.05em' : '-0.01em',
              ...
              {isTempChat && (
                ...
                >
                  Your conversation is private and won't be saved.<br />
                  Messages disappear when you leave this session.
                </motion.p>
              )}
            </div>
          </div>
          ...
          <div
            style={{
              maxWidth: isSidebarOpen ? 760 : 840,
              width: '100%',
              margin: '0 auto',
              padding: '0 20px',
              display: 'flex',
              flexDirection: 'column', // MATCH PARENT
              gap: 2,
              transition: 'max-width 0.3s ease', // SMOOTH EXPANSION
            }}
          >
            <AnimatePresence initial={false}>
              {messages.map((msg, i, arr) => {
                const chronoPrev = arr[i - 1]
                const isFirst = !chronoPrev || chronoPrev.role !== msg.role
                return (
                  <MessageBubble
                    key={msg.id || i}
                    message={msg}
                    ...
                    isGenerating={isGenerating && i === messages.length - 1 && msg.role === 'assistant'}
                  />
                )
              })}
            </AnimatePresence>

            {/* Typing indicator at the end = bottom visually */}
            <AnimatePresence>
